...
def test_전문가_비용_몫은_배분한_weight에_비례한다() -> None:
    ...
    sends = _dispatch({"plan": plan, "max_cost_usd": 2.0})  # type: ignore[typeddict-item]
...
def test_띄울_전문가가_없으면_즉시_빈_결과로_끝낸다() -> None:
    ...
    empty = _dispatch({"plan": DispatchPlan(), "max_cost_usd": 2.0})  # type: ignore[typeddict-item]
    missing = _dispatch({"plan": None, "max_cost_usd": 2.0})  # type: ignore[typeddict-item]
...
def test_남은_예산이_없으면_전문가를_띄우지_않고_끝낸다() -> None:
    ...
    sends = _dispatch({"plan": plan, "max_cost_usd": 2.0, "model_cost_usd": 2.0})  # type: ignore[typeddict-item]
