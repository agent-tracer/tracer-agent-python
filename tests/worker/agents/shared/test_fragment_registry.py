"""LangGraph 프롬프트 조각 레지스트리와 조립 결과를 고정한다."""
...
from tracer_agent.worker.agents.chat.prompts import LAN_ASSISTANT_SYSTEM_PROMPT
from tracer_agent.worker.agents.recipe_scan.prompt_fragments import LAN_RECIPE_SCAN_FRAGMENT_REGISTRY
from tracer_agent.worker.agents.recipe_scan.prompts import (
    LAN_INVESTIGATOR_SYSTEM_PROMPT as LAN_RECIPE_INVESTIGATOR_SYSTEM_PROMPT,
)
from tracer_agent.worker.agents.recipe_scan.prompts import LAN_PROBE_SYSTEM_PROMPT, LAN_SURVEY_SYSTEM_PROMPT
...
from tracer_agent.worker.agents.task_cleanup.prompts import (
    LAN_INVESTIGATOR_SYSTEM_PROMPT as LAN_CLEANUP_INVESTIGATOR_SYSTEM_PROMPT,
)
...
from tracer_agent.worker.agents.title_suggestion.prompts import (
    LAN_INVESTIGATOR_SYSTEM_PROMPT as LAN_TITLE_INVESTIGATOR_SYSTEM_PROMPT,
)

_INTEGRITY_CONTRACT = shared_contract("prompt.fragment.integrity.json")

_REGISTRIES = {
    "chat": LAN_CHAT_FRAGMENT_REGISTRY,
    "recipe-scan": LAN_RECIPE_SCAN_FRAGMENT_REGISTRY,
    "task-cleanup": LAN_TASK_CLEANUP_FRAGMENT_REGISTRY,
    "title-suggestion": LAN_TITLE_SUGGESTION_FRAGMENT_REGISTRY,
}
...
_PROMPT_HASHES = {
    ...
    "cleanup.investigator": (
        LAN_CLEANUP_INVESTIGATOR_SYSTEM_PROMPT,
    ...
    "title.investigator": (
        LAN_TITLE_INVESTIGATOR_SYSTEM_PROMPT,
...
def test_모든_프래그먼트가_LAN_식별자와_완전한_메타데이터를_갖는다(agent: str) -> None:
    for code_name, fragment in _REGISTRIES[agent].items():
        assert code_name.startswith("LAN_")
        assert fragment.code_name == code_name
        assert fragment.definition_key.startswith(f"lan.{agent}.")
        ...
        assert all(key.startswith(f"lan.{agent}.") for key in fragment.template_keys)


def test_자리표시자와_사용_프롬프트가_프래그먼트에_기록된다() -> None:
    fragment = LAN_TASK_CLEANUP_FRAGMENT_REGISTRY["LAN_SUGGESTION_RULES"]
    assert fragment.placeholders == ("maxEvidenceEventIds",)
    assert fragment.template_keys == ("lan.task-cleanup.investigator.system",)
