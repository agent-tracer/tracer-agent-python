...
- 소비자 그룹 이름과 배출기의 자문 잠금 열쇠는 계약의 template 과 axisOffset 에서 자기 축으로 파생시킵니다. 완성된 이름과 수를 소스에 적으면 `compare` 프로파일에서 한 축이 다른 축의 진행과 잠금을 막습니다.
- 배경 작업이 만드는 행은 `backend`에 자기 축 상수를 적고 그 행을 읽는 자리는 자기 축의 행만 봅니다. 걸러야 하는 자리의 목록은 계약의 `conformance/cases/recipe.ledger.json`의 `axisScope`가 갖습니다.
- 색인 문서의 식별자는 축과 레시피 식별자를 잇고 검색은 문서의 `backend` 칸으로 자기 축의 문서만 봅니다. 적중의 레시피 식별자는 문서 식별자를 잘라 쓰지 않고 문서의 `recipeId` 칸에서 읽습니다.
