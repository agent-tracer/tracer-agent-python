...
- 구현 차이는 예외 목록으로 숨기지 않고 `divergence.json`과 함께 갱신합니다. 이 저장소만으로 닫을 수 없어 미룬 항목은 `CONTRACT-BACKLOG.md`에 모으고 흩어 두지 않습니다.
...
- 접수는 원장 연결을 쥔 채 다른 창구를 부르지 않습니다. 한 요청이 같은 풀에서 연결 둘을 요구하면 그 풀은 자기 자신을 기다립니다.
