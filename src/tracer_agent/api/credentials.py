"""사용자 설정 원장에 둔 모델 자격과 고른 모델을 실행 봉투 창구에 평문으로 낸다."""

from __future__ import annotations

from ..shared.agents.envelope.models import API_KEY_SETTING, MODEL_SETTING
...
class SettingModelCredentials:
    """설정 원장이 든 모델 자격과 고른 모델을 그 사용자 범위에서 찾아 낸다."""
    ...
    async def api_key(self, user_id: str) -> str | None:
        ...
        return await self._read(user_id, API_KEY_SETTING)

    async def chosen_model(self, user_id: str) -> str | None:
        """그 사용자가 고른 모델이며 고르지 않았으면 None이다."""
        return await self._read(user_id, MODEL_SETTING)

    async def _read(self, user_id: str, key: str) -> str | None:
        async with self._source.connect() as sql:
            stored = await AppSettingStore(sql, self._cipher).list_by_scope(user_id)
        return next((one.value for one in stored if one.key == key), None)
