...
from ...shared.agents.chat.models import GRAPH_BACKEND
...
async def resume_active_executions(
    ...
    """갱신이 끊긴 running을 되돌린 뒤 이 백엔드가 소유한 활성 실행을 모두 다시 신호한다."""
    ...
    async with sql.connect() as connection:
        ...
        active = await ledger.list_active_on_backend(GRAPH_BACKEND)
    for row in active:
