...
from ..runtime.scoped_event_reader import OPTIONAL_EVENT_KEYS, ScopedEventReader, slim_event
...
class RecipeLedgerReader(ScopedEventReader):
    """이 축의 이벤트 표현으로 읽고, 요약이 보는 태스크와 규칙까지 함께 읽는다."""

    def __init__(self, tracer: TracerApiPort) -> None:
        super().__init__(tracer, slim_recipe_event)
    ...
    async def task_with_events(self, task_id: str, window: int) -> TaskEventWindow | None:
        ...
        if detail is None:
            return None
        read = await self.event_window(task_id, window)
        if read is None:
            return None
        rows, total = read
