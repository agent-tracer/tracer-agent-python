...
async def deliver_recipes(tracer: TracerApiPort, execution_id: str, data: dict[str, Any]) -> bool:
    ...
    body = {
        "recipes": [_to_draft(recipe, revisions) for recipe in recipes[:MAX_RECIPES]],
        "author": AGENT_AUTHOR,
...
def _to_draft(candidate: Mapping[str, Any], revisions: Mapping[str, Any]) -> dict[str, Any]:
    """후보 하나를 창구가 받는 draft로 옮기며 창구가 갖지 않는 칸은 싣지 않는다."""
    draft = {wire: candidate[name] for name, wire in _DRAFT_FIELDS if name in candidate}
