...
class ScanAnchorClient:
    ...
    async def find(self, user_id: str, task_id: str) -> ScanAnchor | None:
        ...
        try:
            ...
            raise AnchorUnavailable(f"scan anchor unreachable: {unreachable}") from unreachable
        if response.status_code == NOT_FOUND_STATUS:
            return None
        if response.status_code >= 400:
            raise AnchorUnavailable(f"scan anchor HTTP {response.status_code}: {response.text[:500]}")
